...
<%
    ...
    if(cat_id == null || "".equals(cat_id) || "null".equals(cat_id))
    {
        ...
        String childs = CategoryManager.getAllChildCategoryIDS(cat_id,site_id);
...
{		
    ...
    if(cat_id != "0")
    {
        mp.put("cat_ids",cat_id);
    }
    mp.put("orderby","ci.released_dtime desc");

	beanList = CmsCountRPC.getInfoListByCat(mp);
...
function  outFileBtn(){
    if(beanList.size() != 0) {
        ...
        listHeader.add("部门");
        listHeader.add("评级");
        var url = CmsCountRPC.getInfoListByCatOutExcel(listHeader,mp,"栏目信息统计表");
        location.href=url;
    }
}
</script>

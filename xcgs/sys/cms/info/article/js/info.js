...
function initTable(){
    ...
    colsList.add(setTitleClos("info_level","稿件评级","70px","","",""));
...
//在信息列表页中 可以直接修改评级 -- start
function setInfoLevel(info_level,info_id){
    var info_level_n = $("#info_level_"+info_id).val();
    if(info_level_n!=info_level){
        //alert(weight_o+"---"+info_id+"----"+weight_n);
        jsonrpc.InfoBaseRPC.updateInfoLevel(updateInfoWeightResult,info_id,info_level_n);
    }
}
...
//	M.PUT("IS_HOST", APP);
    ...
    table.getCol("weight").each(function(i){
		...
		{	
			...
			$(this).html('<input type="text" name="weight" id="info_'+info_id_o+'" value="'+weight_o+'" style="width:20px;text-align:center;" onblur="setInfoWeight('+weight_o+','+info_id_o+')"/>');
		}
	});
    //在信息列表页中 可以直接修改权重 -- end

    //在信息列表页中 可以直接修改权重 -- start
    table.getCol("info_level").each(function(i){
        //$(this).css({"text-align":"left"});
        if(i>0)
        {
            var info_level_o = beanList.get(i-1).info_level;
            var info_id_o = beanList.get(i-1).info_id;
            $(this).html('<select name="info_level" id="info_level_'+info_id_o+'"  onchange="setInfoLevel(\''+info_level_o+'\','+info_id_o+')"><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option></select>');
            $("#info_level_"+info_id_o).val(info_level_o);
        }
    });
    //在信息列表页中 可以直接修改权重 -- end
...
function toPass(){
    ...
    //打开选择审核人页面
    top.OpenModalWindow("选择审核人","/sys/cms/info/article/chooseAudit.jsp?cat_id="+cid+"&site_id="+site_id+"&app_id="+app,400,200);
    /*
	var selectList = table.getSelecteBeans();	
...
function doPass(num){
    //打开选择审核人页面
    top.OpenModalWindow("选择审核人","/sys/cms/info/article/chooseAudit.jsp?cat_id="+cid+"&site_id="+site_id+"&app_id="+app+"&num="+num,400,200);
    /*
	var list = new List();

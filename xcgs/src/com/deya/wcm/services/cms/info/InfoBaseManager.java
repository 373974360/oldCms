...
public class InfoBaseManager {
	...
	public static void getSearchSQL(Map<String, String> map){
		...
		if(map.containsKey("cat_ids"))
		{
            ...
            if(ids != null && ids.length() > 0){
                ...
                {
                    if(CategoryReleManager.isCategoryManagerByUser(Integer.parseInt(user_id),map.get("site_id"),Integer.parseInt(temp)))
                    {
     ...
     * @return String
     */
    public static String getInfoTemplateContent(String info_id,String site_id,String cur_page)
    {
        String content = "";
        InfoBean ib = getInfoById(info_id,site_id);
        if(ib != null)
        {
            ...
            int model_id = ib.getModel_id();
            String template_id = CategoryModelManager.getTemplateID(cat_id+"",ib.getSite_id(),model_id);
            VelocityInfoContextImp vici = new VelocityInfoContextImp(ib.getInfo_id()+"",temp_site_id,template_id,model_ename,cur_page);
            content = vici.parseTemplate();
        }
        return content;
    }

...
import com.deya.wcm.services.org.dept.DeptRPC;
    ...
    **************************************************************************************************************************************/
    ...
    public static List<SiteCountBean> getCatListByReleaseDtime(Map<String, String> mp){
        ...
        try{
            ...
            listResult = getReleaseCountListByCate(mp,catSet);
     ...
     * @return	取得所选栏目下的工作量统计信息列表
     */
    public static List<SiteCountBean> getReleaseCountListByCate(Map<String, String> mp,Set<String> catSet){
        List<SiteCountBean> resultList = new ArrayList<SiteCountBean>();
        try{
            Map<String, CmsCountBean> all_m = CmsCountDAO.getCatListByReleaseDtime(mp);

            List<CmsCountBean> ret = new ArrayList<CmsCountBean>();
            for(String s : all_m.keySet()){
                CmsCountBean bean = all_m.get(s);
                bean.setInputCount(bean.getCount());
                ret.add(all_m.get(s));
            }
            Collections.sort(ret, new CntDescComparator());
    ...
    public static String getInfoListByCatOutExcel(List headerList,Map mp,String titleName){
        try{
            ...
            List<InfoBean> listBean = CmsCountManager.getInfoListByCat(mp);
            ...
            for(int i=0;i<listBean.size();i++){
                InfoBean info = (InfoBean)listBean.get(i);
                data[i][0] = String.valueOf(i + 1);
                data[i][1] = String.valueOf(info.getReleased_dtime());
                data[i][2] = String.valueOf(info.getTitle());
                data[i][3] = String.valueOf(info.getCat_cname());
                data[i][4] = String.valueOf(info.getAuthor());
                data[i][5] = String.valueOf(info.getSource());
                data[i][6] = String.valueOf(info.getInfo_level());
            }

            OutExcel oe=new OutExcel(titleName);
    ...
    public static String getInfoListByDeptOutExcel(List headerList,Map mp,String titleName){
        try{
            ...
            List<InfoBean> listBean = CmsCountManager.getInfoListByDept(mp);
     ...
     * 取得所选部门下的统计信息
     * @param mp
     * @return	取得所选部门下的统计信息列表
     */
    public static List<SiteCountBean> getDeptListByReleaseDtime(Map<String, String> mp){

        List<SiteCountBean> listResult = new ArrayList<SiteCountBean>();
        try{
            String dept_ids = (String)mp.get("dept_ids");
            if(dept_ids==null){
                dept_ids = "";
            }
            Set<String> deptSet = new HashSet<String>();
            if(!dept_ids.equals("")){
                List<String> deptList = Arrays.asList(dept_ids.split(","));
                for(String dept_id : deptList){
                    if(dept_id!=null && !"".equals(dept_id)){
                        DeptBean deptBean = DeptManager.getDeptBeanByID(dept_id);
                        String dept_position = deptBean.getTree_position();
                        List<String> catpList = Arrays.asList(dept_position.split("\\$"));
                        for(String deptp_id : catpList){
                            if(deptp_id!=null && !"".equals(deptp_id)){
                                deptSet.add(deptp_id);
                            }
                        }
                    }
                }
            ...
            listResult = getReleaseCountListByDept(mp,deptSet);
     ...
     * 取得所选部门下的工作量统计信息
     * @param mp
     * @return	取得所选部门下的工作量统计信息列表
     */
    public static List<SiteCountBean> getReleaseCountListByDept(Map<String, String> mp,Set<String> deptSet){
        List<SiteCountBean> resultList = new ArrayList<SiteCountBean>();
        try{
            Map<String, CmsCountBean> all_m = CmsCountDAO.getDeptListByReleaseDtime(mp);
            ...
            String site_id = (String)mp.get("site_id");
            String treeStr = DeptRPC.getAllDeptTreeJsonStr();
            JSONArray jsonArray = JSONArray.fromObject(treeStr);
            Iterator it = jsonArray.iterator();
            while(it.hasNext()){
                JSONObject jsonObject = (JSONObject)it.next();
                SiteCountBean  siteCountBean = doOutTreeBeanByDept(jsonObject,ret,deptSet);
                resultList.add(siteCountBean);
            }
            return resultList;
        }catch (Exception e) {
    ...
    //getReleaseCountListByDept  -- 调用的递归方法用
    public static SiteCountBean doOutTreeBeanByDept(JSONObject jsonObject,List<CmsCountBean> list,Set<String> deptSet){
        SiteCountBean siteCountBean = new SiteCountBean();
        try{
            String str = jsonObject.toString();
            JSONArray jsonArray = JSONArray.fromObject(jsonObject.get("children"));
            if(!jsonArray.toString().equals("[null]") && jsonArray!=null && !"".equals(jsonArray) && jsonArray.size()>0){//有子节点
                int dept_id = Integer.valueOf(String.valueOf(jsonObject.get("id")));
                //判断选没有选该栏目
                if(deptSet.size()>0 && !deptSet.contains(dept_id+"")){
                    return null;
                }
                siteCountBean.setDept_id(dept_id);
                siteCountBean.setIs_leaf(false);
                siteCountBean.setDept_name(String.valueOf(jsonObject.get("text")));
                int inputCount = 0;
                int releasedCount = 0; //发布信息条数
                int picReleasedCount = 0; //发布信息条数 并且有图片
                Iterator it = jsonArray.iterator();
                ...
                while(it.hasNext()){
                    ...
                    if(object!=null){
                        JSONObject jsonObject2 = (JSONObject)object;
                        if(jsonObject2!=null){
                            SiteCountBean countBean = doOutTreeBeanByDept(jsonObject2,list,deptSet);
                            if(countBean!=null){
                                inputCount += countBean.getInputCount();
                                releasedCount += countBean.getReleasedCount();
                                picReleasedCount += countBean.getPicReleasedCount();
                ...
                int picReleasedCount = 0; //发布信息条数 并且有图片
                for(CmsCountBean cmsCountBean : list){
                    if(dept_id==cmsCountBean.getCat_id()){
                        inputCount += cmsCountBean.getInputCount();
                        releasedCount += cmsCountBean.getReleasedCount();
                        picReleasedCount += cmsCountBean.getPicInfoCount();
                        break;
     ...
     * 取得所选部门下的详细信息列表
     * @param mp
     * @return	取得所选部门下的详细信息列表
     */
    public static List<InfoBean> getInfoListByDept(Map<String, String> mp){

        return CmsCountDAO.getInfoListByDept(mp);
    }

    /**
     * 取得所填作者下的工作量统计信息
     * @param mp
     * @return	取得所填作者下的工作量统计信息
     */
    public static List<SiteCountBean> getAuthorListByReleaseDtime(Map<String, String> mp){
        List<SiteCountBean> resultList = new ArrayList<SiteCountBean>();
        try{
            /*
            String author = mp.get("author");
            String inputAuthor = "'";
            if(author != null && !"".equals(author))
            {
                String[] authorStr = author.split(",");
                for(String temp_author : authorStr)
                {
                    inputAuthor = inputAuthor + temp_author + "','";
                }
                if(inputAuthor.length() > 2)
                {
                    inputAuthor = inputAuthor.substring(0,inputAuthor.length() - 2);
                }
                mp.remove("author");
                mp.put("author",inputAuthor);
            }
            */
            Map<String, CmsCountBean> all_m = CmsCountDAO.getAuthorListByReleaseDtime(mp);
            ...
            Collections.sort(ret, new CntDescComparator());
            for(CmsCountBean ccb : ret)
            {
                SiteCountBean siteCountBean = new SiteCountBean();
                siteCountBean.setUser_name(ccb.getUser_name());
                siteCountBean.setInputCount(ccb.getCount());
                siteCountBean.setReleasedCount(ccb.getReleasedCount());
                siteCountBean.setPicReleasedCount(ccb.getPicInfoCount());
                resultList.add(siteCountBean);
            }
            return resultList;
        }catch (Exception e) {
    ...
    public static List<InfoBean> getInfoHitsByReleaseDtime(Map<String, String> mp){
        return CmsCountDAO.getInfoHitsByReleaseDtime(mp);
    }
    ...
    public static List<SiteCountBean> getCatHitsByReleaseDtime(Map<String, String> mp){
        List<SiteCountBean> resultList = new ArrayList<SiteCountBean>();
        try{
            Map<String, CmsCountBean> all_m = CmsCountDAO.getCatHitsByReleaseDtime(mp);
            List<CmsCountBean> ret = new ArrayList<CmsCountBean>();
            for(String s : all_m.keySet()){
                CmsCountBean bean = all_m.get(s);
                bean.setInputCount(bean.getCount());
            ...
            {
                SiteCountBean siteCountBean = new SiteCountBean();
                siteCountBean.setCat_id(ccb.getCat_id());
                siteCountBean.setCat_name(ccb.getCat_name());
                siteCountBean.setInputCount(ccb.getCount());
                siteCountBean.setReleasedCount(ccb.getReleasedCount());
                siteCountBean.setPicReleasedCount(ccb.getPicInfoCount());
                resultList.add(siteCountBean);
    ...
    public static String getInfoHitsOutExcel(List headerList,Map mp,String titleName){
        try{
            ...
            List<InfoBean> listBean = getInfoHitsByReleaseDtime(mp);
            ...
            for(int i=0;i<listBean.size();i++){
                ...
                data[i][5] = String.valueOf(info.getSource());
                data[i][6] = String.valueOf(info.getInfo_level());
                data[i][7] = String.valueOf(info.getHits());
            }
    ...
    public static String getCatHitsOutExcel(List headerList,Map mp,String titleName){
        try{
            ...
            List<SiteCountBean> listBean = getCatHitsByReleaseDtime(mp);
            ...
            String[][] data = new String[listBean.size()][8];
            for(int i=0;i<listBean.size();i++){
                SiteCountBean info = listBean.get(i);
                data[i][0] = String.valueOf(i + 1);
                data[i][1] = String.valueOf(info.getCat_name());
                data[i][2] = String.valueOf(info.getInputCount());
            }

...
public class UserLogin {
	...
	public static String checkUserLogin(String user_name,String pass_word,HttpServletRequest request)
	{
		...
		if("0".equals(ms))
		{
            ...
            request.getSession().setAttribute("cicro_wcm_user",lub);
	...
	public static boolean loginOut(HttpServletRequest request){
        ...
        request.getSession().removeAttribute("cicro_wcm_user");
		return true;
	}
	...
	public static LoginUserBean getUserBySession(HttpServletRequest request)
	{
        ...
        HashMap allSession = MySessionContext.getAllSession();
        Set<String> myset = allSession.keySet();
        HttpSession session = request.getSession();
        if(session != null)
        {
            for(String sessionId : myset)
            {
                HttpSession mySession = (HttpSession) allSession.get(session.getId());
                if(mySession != null) {
                    try {
                        Object obj = mySession.getAttribute("cicro_wcm_user");
                        if (obj != null) {
                            userBean = (LoginUserBean) obj;
                        }

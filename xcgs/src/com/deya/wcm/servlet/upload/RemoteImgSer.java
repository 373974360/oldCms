...
public class RemoteImgSer extends HttpServlet{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {    
			...
			if (src.lastIndexOf(".") >= 0) {
				extName = src.substring(src.lastIndexOf("."))
						.toLowerCase();
	...
	public static boolean processImage(File file, BufferedImage bis, String name,
			String extName, String savePath,String is_press,String press_osition,String site_id,String fileUrl) throws IOException {
		try {				
			File saveFile;			
			int w = bis.getWidth();
			...
			if (w > content_img_width && "1".equals(is_compress)) {
				...
				if (extName.equals(".gif"))
					ImageUtils.CreatetHumbnailByGif(saveFile, file,
							ImageUtils.getImgWidth());
				else
					ImageUtils.CreatetHumbnail(saveFile, file, ImageUtils
							.getImgWidth());
			...
			if(w > prev_img_width)
			{
				...
				if (extName.equals(".gif"))
					ImageUtils.CreatetHumbnailByGif(saveFile, humbImg,
							prev_img_width);
				else
					ImageUtils.CreatetHumbnail(saveFile, humbImg, ImageUtils
							.getImgPreWidth());
			}
			return true;
		} catch (Exception e) {

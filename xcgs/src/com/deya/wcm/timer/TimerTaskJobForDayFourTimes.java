 ...
 *
 */
public class TimerTaskJobForDayFourTimes implements Job{
	...
	public TimerTaskJobForDayFourTimes(){

	}

	static{
		try{			
			String[] class_arr = bc.getPropertyNamesByCategory("timerlistener_day_class_four_times");
			if(class_arr != null && class_arr.length > 0)
        	{
				...
				for(int i=class_arr.length;i > 0;i--)
        		{
					...
					Class.forName(bc.getProperty(class_arr[i-1], "", "timerlistener_day_class_four_times"));
        		}
        	}

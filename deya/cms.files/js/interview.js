...
$(document).ready(function () {	
	...
	{
		$("#sandMess").parent().css("display","none");
...
function getForecastVideo(sta)
{
	...
	{
		if(img_path.trim() == ""){
			img_path = "/sys/images/default_live.jpg";
		}
...
function setUserInfo()
{
	...
	{//所有用户都可以发言
		...
		$("#sandMess").unbind("click").click(addMessage);
	}
		...
		if(srList.size() > 0)
		{
			...
			for(var i=0;i<srList.size();i++)
			{
				if(srList.get([i]) != null)
				{
					...
					var str = '<li><a href="'+srList.get([i]).affix_path+'" class="group1" title="'+p_name+'" ><img src="'+srList.get([i]).affix_path+'" ></a><div>'+p_name+'</div></li>';
					$("#imgLive").append(str);
				}
			}
...
{
	var m = ChatRPC.getAllMessageList(sub_id,pic_start_num,text_start_num,false);
...
function setVideoPath(live_video)
{
    ...
    if(live_video.indexOf(".flv")>0 || live_video.indexOf(".mp4")>0)
	{
		jwplayer("live_video_div").setup({
			...
			file: live_video,
			autostart:true,
			width:$("#live_video_div").width(),
			height:$("#live_video_div").height(),
			image: ""
		});
	}
	else
	{
		$("#live_video_div").html('<embed id="live_video_embed" width="'+$("#live_video_div").width()+'" height="'+$("#live_video_div").height()+'" src="'+live_video+'" mediatype="video" autostart="true" loop="true" menu="true" sck_id="0" type="application/x-mplayer2" ></embed>');
		...
		{			
			...
			{//没有直播视频，查看是否有预告视频
				...
				else
				{
					...
					{
						forecast_pic = "/sys/images/default_live.jpg";
					}

...
//按中文字符截字符串
String.prototype.subString2 = function(maxnum){
	var len = 0;
	var ret=true;
	var pos = this.length;
	var retStr;
...
//字符排重，去掉第一个参数中已有的数据
function removeAlreadyStr(str,old_str)
{
	var arr1 = str.split(",");
	var arr2 = old_str.split(",");
...
//字符排重，得到第一个参数没有的数据
function removeNotAlreadyStr(str,old_str)
{
...
//判断时间大小
function judgeAllDateTime(start_date,end_date)
{
	var reg_ymd=/^([\d]{4})-([\d]{1,2})-([\d]{1,2})\s([\d]{1,2}):([\d]{1,2}):([\d]{1,2})$/;
	var arr_dt1,arr_dt2,dt1,dt2;

	arr_dt1=start_date.match(reg_ymd);
	arr_dt2=end_date.match(reg_ymd);

	dt1=new Date(arr_dt1[1],arr_dt1[2],arr_dt1[3],arr_dt1[4],arr_dt1[5],arr_dt1[6]);
	dt2=new Date(arr_dt2[1],arr_dt2[2],arr_dt2[3],arr_dt2[4],arr_dt2[5],arr_dt2[6]);

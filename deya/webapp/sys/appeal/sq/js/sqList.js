...
{
    getAppealSQCountByURL(window.location.href);
}
...
{
    //addTab(true,"/sys/appeal/sq/sq_add.jsp?sq_id="+s_id+"&site_id="+site_id+"&top_index="+curTabIndex,"信件管理");
    window.location.href = "/sys/appeal/sq/sq_add.jsp?sq_id="+s_id+"&site_id="+site_id;
}

function openSQManager2(s_id)
{
    //addTab(true,"/sys/appeal/sq/sq_add2.jsp?sq_id="+s_id+"&site_id="+site_id+"&top_index="+curTabIndex,"信件管理");
    window.location.href = "/sys/appeal/sq/sq_add.jsp?sq_id="+s_id+"&site_id="+site_id;
}
...
function insertProcess(pro_type)
{
    ...
    switch(pro_type)
    {
        ...
        {
            //根据人员ID得到下一步的步骤ID
            var step_id = jsonrpc.WorkFlowRPC.getNextStepIDByUserID(model_bean.wf_id,LoginUserBean.user_id,"appeal","");
            if(step_id == 100)
            {//办结完了
        ...
        case 8:sqMap.put("limit_flag",2);//延时申请标识 2：申请延时
            sqMap.put("is_back",0);
            break;
        case 101:if($(":radio[id='auto_status']:checked").val() == 9) //延期审核
        {//审核通过
        ...
        else
        {
            ...
            break;
        case 13:sqMap.put("supervise_flag",$(":radio[id=supervise_flag]:checked").val());//督办状态
            break;
        case 102:sqMap.put("publish_status",$(":radio[id='publish_status']:checked").val()+"");//发布标识
            sqMap.put("is_open",$(":radio[id='is_open_r']:checked").val());
    ...
    if(is_submit)
    {
        ...
        {
            try{
                getCurrentFrameObj(top_index).reloadSQList();
                getCurrentFrameObj(top_index).updateTopTreeCount();
            }catch(e){}
        }
        else
        {
            try{
                getCurrentFrameObj(top_index).getProcessList();
                getCurrentFrameObj(getCurrentFrameObj(top_index).top_index).updateTopTreeCount();
                getCurrentFrameObj(getCurrentFrameObj(top_index).top_index).reloadSQList();
            }catch(e){}
        }
        tab_colseOnclick(curTabIndex);
    }else
    {
        parent.msgAlert(WCMLang.Add_fail);
    }
}
//得到业务列表
...
function getModelListSByUserID(){
    p_list = SQModelRPC.getModelListSByUserID(LoginUserBean.user_id);
    p_list = List.toJSList(p_list);
    $("#model_id").addOptions(p_list,"model_id","model_cname");
...
function deleteSQ()
{
    ...
    {
        parent.msgAlert("信件"+WCMLang.Delete_success);
        reloadSQList();
    }else
    {
        msgWargin("信件"+WCMLang.Delete_fail);
    }
}
...
{
    msgConfirm("确认收回此信件？","withdrawSQForProHandl()");
}
...
function withdrawSQForProHandl()
{
    ...
    var return_val = SQRPC.withdrawSQForProcess(selectIDS,LoginUserBean.user_id);
    if(return_val == "true")
    {
        parent.msgAlert("信件收回成功，请在待处理信件列表中重新处理");
        reloadSQList();
    }
    if(return_val == "false")
    {
        msgWargin("信件收回失败，请重新进行收回操作");
        return;
    }
    if(return_val == "process_is_worked")
    {
        msgWargin("该信件已被处理，不能进行收回操作");
        return;
    }
}
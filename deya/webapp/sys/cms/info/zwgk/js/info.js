...
function showList(){		
	...
	beanList = InfoBaseRPC.getGKBZHInfoList(m);//第一个参数为站点ID，暂时默认为空
...
function showTurnPage(){	
	...
	tp.total = InfoBaseRPC.getGKBZHInfoCount(m);
	tp.show("turn","");	
...
function deleteInfoData(){
	...
	if(InfoBaseRPC.deleteGKBZHInfo(selectList)){
...
function openWindowForMov(){	
	top.OpenModalWindow("信息转移","/sys/cms/info/zwgk/infoMove.jsp?site_id="+site_id+"&app_id="+app,400,460);
}
...
function moveInfoHandl(cat_id)
{
	...
	return InfoBaseRPC.MoveGKBZHInfo(new_list,cat_id);
}
...
function doDeleteHandl(num){
	var list = new List();
	list.add(beanList.get(num));
	if(InfoBaseRPC.deleteGKBZHInfo(list)){
		top.msgAlert("信息"+WCMLang.Delete_success);
		reloadInfoDataList();
	}else{
		top.msgWargin("信息"+WCMLang.Delete_fail);
...
function getInfoFromOtherCat(){
	//打开页面
	is_save_first_page = true;
	top.OpenModalWindow("信息获取","/sys/cms/info/zwgk/infoGet.jsp?cat_id="+cid+"&app_id="+app,800,530);
}

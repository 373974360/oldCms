...
function getFirstChileCateID() {
    ...
    if ($('#leftMenuTree .tree-node-selected').find('.tree-folder').length == 0) {
        ...
        cid = getFirstChildNodeID();
        $("#cat_id").val(cid);
        //$("#showCatId").text(getFirstChildNodeValue());
    }
}
...
function addRelatedHand() {
    ...
    OpenModalWindow("手动添加", "/sys/cms/info/article/addRelatedInfo.jsp?info_id=" + infoid + "&site_id=" + site_id, 400, 200);
}
...
function openLinkInfoDataPage(Info_id, t_site_id) {
    ...
    if (t_site_id != "" && t_site_id != null) {
        ...
        temp_site = site_id;
    OpenModalWindow("选择相关信息", "/sys/cms/info/article/chooseInfoPage.jsp?info_id=" + Info_id + "&site_id=" + temp_site, 825, 500);
}
...
function openFocusPage(Info_id, t_site_id) {
    ...
    if (app_id == "zwgk") {
        ...
        temp_site = site_id;
    OpenModalWindow("选择手动标签", "/sys/cms/info/article/focusInfo.jsp?info_id=" + Info_id + "&site_id=" + temp_site + "&t=" + tmp, 660, 500);
}
...
function insertOtherInfos(bean, model_ename, info_id) {
    ...
    if (catIds != "") {
        ...
        for (var i = 0; i < ids.length; i++) {//都以引用方式过去
...
function setUserOperate() {
    var opt_ids = "," + getOptIDSByUser(app_id, site_id) + ",";//登录人所拥有管理权限ID
    userOpts = opt_ids;
    //判断是否是站点管理员或超级管理员

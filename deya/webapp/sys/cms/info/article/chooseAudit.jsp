...
<head>
    ...
    <script type="text/javascript">
                ...
                for(var i=0; i<workStepList.size(); i++){
                    if(workStepList.get(i).step_id > step_id)
                    {
                        if(workStepList.get(i).required == 1)
                        {
                            ...
                            var html = '<input id="auditChecked" name="auditStep" type="radio" checked="checked" onclick="setStepId(' + (workStepList.get(i).step_id - 1) + ',2)"/><label for="e">' + workStepList.get(i).step_name + '</label>&nbsp;&nbsp;';
                        ...
                        {
                            $("#audit_tr").removeClass("hidden");
                            var html = '<input name="auditStep" type="radio" onclick="setStepId(' + (workStepList.get(i).step_id - 1) + ',2)"/><label for="e">' + workStepList.get(i).step_name + '</label>&nbsp;&nbsp;';
                            $("#audit_list").append(html);
                        }
                    }
                }
                if(top.top.getCurrentFrameObj().opt_ids.indexOf("302") > -1)
                {
                    $("#audit_tr").removeClass("hidden");
                    $("#audit_list input").removeAttr("checked");
                    var html = '<input name="auditStep" type="radio" onclick="setStepId(100,8)" checked="checked"/><label for="e">发布</label>&nbsp;&nbsp;';
                    info_step_id = 100;
                    info_status = 8;
                    $("#audit_list").append(html);
                }
        ...
        function setStepId(value,status)
        {
            info_step_id = value;
            info_status = status;
        }
        ...
        function related_ok()
        {
            ...
            {
                list.add(top.top.getCurrentFrameObj().beanList.get(num));
            }
            else
            {
                ...
                for(var i = 0; i < info_list.size(); i++)
                {
                    ...
                    infoBean.info_status = info_status;
                    list.add(infoBean);
                }
            }
            if(info_status == 8)
            {
                if(InfoBaseRPC.updateInfoStatus(list,"8"))
                {
                    ...
                    top.top.getCurrentFrameObj().reloadInfoDataList();
                }
                else{
                    top.msgWargin("发布操作失败");
                }
            }
            else
            {

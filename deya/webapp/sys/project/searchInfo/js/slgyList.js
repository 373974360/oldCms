...
function openUpdateSlgyPage(id) {
    ...
    OpenModalWindow("森林公园信息", "/sys/project/searchInfo/viewSlgy.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 453);
}
...
function deleteSlgy() {
    ...
    if (SlgyRPC.deleteSlgy(m)) {
        msgAlert("森林公园信息" + WCMLang.Delete_success);
        reloadSlgyList();
    }
    else {
        msgWargin("森林公园信息" + WCMLang.Delete_fail);
    }
}
...
function updateSlgyData() {
    ...
    if (bool) {
        msgAlert("森林公园信息保存成功");
        getCurrentFrameObj(topnum).reloadSlgyList();
        CloseModalWindow();
    } else {
        msgWargin("森林公园信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("森林公园信息", "/sys/project/searchInfo/viewSlgy.jsp?topnum="+curTabIndex, 485, 453);
}
...
function changeStatus(value) {
    ...
    if (SlgyRPC.changeStatus(m)) {
        msgAlert("森林公园信息状态设置成功");
        reloadSlgyList();
    } else {
        msgWargin("森林公园信息状态设置失败，请重新操作");
    }
}

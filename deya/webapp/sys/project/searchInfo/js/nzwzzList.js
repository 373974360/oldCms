...
function openUpdateNzwzzPage(id) {
    ...
    OpenModalWindow("农作物种子生产经营企业信息", "/sys/project/searchInfo/viewNzwzz.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 453);
}
...
function deleteNzwzz() {
    ...
    if (NzwzzRPC.deleteNzwzz(m)) {
        msgAlert("农作物种子生产经营企业信息" + WCMLang.Delete_success);
        reloadNzwzzList();
    }
    else {
        msgWargin("农作物种子生产经营企业信息" + WCMLang.Delete_fail);
    }
}
...
function updateNzwzzData() {
    ...
    if (bool) {
        msgAlert("农作物种子生产经营企业信息保存成功");
        getCurrentFrameObj(topnum).reloadNzwzzList();
        CloseModalWindow();
    } else {
        msgWargin("农作物种子生产经营企业信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("农作物种子生产经营企业信息", "/sys/project/searchInfo/viewNzwzz.jsp?topnum="+curTabIndex, 485, 453);
}
...
function changeStatus(value) {
    ...
    if (NzwzzRPC.changeStatus(m)) {
        msgAlert("农作物种子生产经营企业信息状态设置成功");
        reloadNzwzzList();
    } else {
        msgWargin("农作物种子生产经营企业信息状态设置失败，请重新操作");
    }
}

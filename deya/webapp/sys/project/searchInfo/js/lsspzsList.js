...
function openUpdateLsspzsPage(id) {
    ...
    OpenModalWindow("绿色食品证书信息", "/sys/project/searchInfo/viewLsspzs.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 483);
}
...
function deleteLsspzs() {
    ...
    if (LsspzsRPC.deleteLsspzs(m)) {
        msgAlert("绿色食品证书信息" + WCMLang.Delete_success);
        reloadLsspzsList();
    }
    else {
        msgWargin("绿色食品证书信息" + WCMLang.Delete_fail);
    }
}
...
function updateLsspzsData() {
    ...
    if (bool) {
        msgAlert("绿色食品证书信息保存成功");
        getCurrentFrameObj(topnum).reloadLsspzsList();
        CloseModalWindow();
    } else {
        msgWargin("绿色食品证书信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("绿色食品证书信息", "/sys/project/searchInfo/viewLsspzs.jsp?topnum="+curTabIndex, 485, 483);
}
...
function changeStatus(value) {
    ...
    if (LsspzsRPC.changeStatus(m)) {
        msgAlert("绿色食品证书信息状态设置成功");
        reloadLsspzsList();
    } else {
        msgWargin("绿色食品证书信息状态设置失败，请重新操作");
    }
}

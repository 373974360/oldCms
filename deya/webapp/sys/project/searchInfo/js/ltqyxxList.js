...
function openUpdateLtqyxxPage(id) {
    ...
    OpenModalWindow("龙头企业信息", "/sys/project/searchInfo/viewLtqyxx.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 433);
}
...
function deleteLtqyxx() {
    ...
    if (LtqyxxRPC.deleteLtqyxx(m)) {
        msgAlert("龙头企业信息" + WCMLang.Delete_success);
        reloadLtqyxxList();
    }
    else {
        msgWargin("龙头企业信息" + WCMLang.Delete_fail);
    }
}
...
function updateLtqyxxData() {
    ...
    if (bool) {
        msgAlert("龙头企业信息保存成功");
        getCurrentFrameObj(topnum).reloadLtqyxxList();
        CloseModalWindow();
    } else {
        msgWargin("龙头企业信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("龙头企业信息", "/sys/project/searchInfo/viewLtqyxx.jsp?topnum="+curTabIndex, 485, 433);
}
...
function changeStatus(value) {
    ...
    if (LtqyxxRPC.changeStatus(m)) {
        msgAlert("龙头企业信息状态设置成功");
        reloadLtqyxxList();
    } else {
        msgWargin("龙头企业信息状态设置失败，请重新操作");
    }
}
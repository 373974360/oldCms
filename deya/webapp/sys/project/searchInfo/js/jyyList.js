...
function openUpdateJyyPage(id) {
    ...
    OpenModalWindow("检验员信息", "/sys/project/searchInfo/viewJyy.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 433);
}
...
function deleteJyy() {
    ...
    if (JyyRPC.deleteJyy(m)) {
        msgAlert("检验员信息" + WCMLang.Delete_success);
        reloadJyyList();
    }
    else {
        msgWargin("检验员信息" + WCMLang.Delete_fail);
    }
}
...
function updateJyyData() {
    ...
    if (bool) {
        msgAlert("检验员信息保存成功");
        getCurrentFrameObj(topnum).reloadJyyList();
        CloseModalWindow();
    } else {
        msgWargin("检验员信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("检验员信息", "/sys/project/searchInfo/viewJyy.jsp?topnum="+curTabIndex, 485, 433);
}
...
function changeStatus(value) {
    ...
    if (JyyRPC.changeStatus(m)) {
        msgAlert("检验员信息状态设置成功");
        reloadJyyList();
    } else {
        msgWargin("检验员信息状态设置失败，请重新操作");
    }
}

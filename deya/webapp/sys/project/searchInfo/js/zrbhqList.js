...
function openUpdateZrbhqPage(id) {
    ...
    OpenModalWindow("自然保护区信息", "/sys/project/searchInfo/viewZrbhq.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 536);
}
...
function deleteZrbhq() {
    ...
    if (ZrbhqRPC.deleteZrbhq(m)) {
        msgAlert("自然保护区信息" + WCMLang.Delete_success);
        reloadZrbhqList();
    }
    else {
        msgWargin("自然保护区信息" + WCMLang.Delete_fail);
    }
}
...
function updateZrbhqData() {
    ...
    if (bool) {
        msgAlert("自然保护区信息保存成功");
        getCurrentFrameObj(topnum).reloadZrbhqList();
        CloseModalWindow();
    } else {
        msgWargin("自然保护区信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("自然保护区信息", "/sys/project/searchInfo/viewZrbhq.jsp?topnum="+curTabIndex, 485, 536);
}
...
function changeStatus(value) {
    ...
    if (ZrbhqRPC.changeStatus(m)) {
        msgAlert("自然保护区信息状态设置成功");
        reloadZrbhqList();
    } else {
        msgWargin("自然保护区信息状态设置失败，请重新操作");
    }
}

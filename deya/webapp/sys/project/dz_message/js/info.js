...
function initTable() {
    ...
    colsList.add(setTitleClos("jimi", "机密等级", "60px", "", "", ""));
    colsList.add(setTitleClos("jinji", "加急等级", "60px", "", "", ""));
...
function showList() {
    ...
    beanList = MessageRPC.getMessageInfoList(m);
    ...
    table.getCol("actions").each(function (i) {
        if (i > 0) {
            ...
            switch (nm) {
                case 1:
                    str += "<li id='549' class='ico_edit'><a title='修改' href='javascript:openUpdateInfoPage(" + beanList.get(i - 1).id + ")' style='width:16px;height:16px;'>&nbsp;&nbsp;&nbsp;&nbsp;</a></li>";
                    str += "<li id='550' class='ico_delete'><a title='删除' href='javascript:doDelete(" + (i - 1) + ")' style='width:16px;height:16px;'>&nbsp;&nbsp;&nbsp;&nbsp;</a></li>";
                    break;
            }
            $(this).html(str + "</ul>");
        }
...
function showTurnPage() {
    ...
    m.put("info_status", infoStatus + '');
    tp.total = MessageRPC.getMessageInfoCount(m);
    tp.show("turn", "");
...
function openAddInfoPage() {
    top.addTab(true, "/sys/project/dz_message/info_add.jsp?topnum=" + top.curTabIndex, "添加通知");
}

function openUpdateInfoPage(id) {
    top.addTab(true, "/sys/project/dz_message/info_add.jsp?topnum=" + top.curTabIndex + "&id=" + id, "修改信息");
}

function openViewPage(id) {
    top.addTab(true, "/sys/project/dz_message/info_view.jsp?topnum=" + top.curTabIndex + "&id=" + id+"&site_id="+site_id, "信息预览");
}
...
function toPass() {
    ...
    if (MessageRPC.updateInfoStatus(selectList, 2)) {
        top.msgAlert("信息发布成功");
        reloadInfoDataList();
    } else {
...
function noPass() {
    var selectList = table.getSelecteBeans();
    if (MessageRPC.updateInfoStatus(selectList, 1)) {
        top.msgAlert("信息发布成功");
        reloadInfoDataList();
    } else {
        top.msgWargin("信息发布失败");
...
function doDeleteHandl(num) {
    ...
    if (MessageRPC.deleteMessage(list)) {
...
function deleteInfoData() {
    var selectList = table.getSelecteBeans();
    if (MessageRPC.deleteMessage(selectList)) {
        top.msgAlert("信息" + WCMLang.Delete_success);
        reloadInfoDataList();
    } else {
        top.msgWargin("信息" + WCMLang.Delete_fail);

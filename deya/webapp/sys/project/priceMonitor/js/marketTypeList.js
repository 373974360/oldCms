...
function openUpdateMarketTypePage(id) {
    ...
    OpenModalWindow("市场分类信息", "/sys/project/priceMonitor/viewMarketType.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 303);
}
...
function deleteMarketType() {
    ...
    if (MarketTypeRPC.deleteMarketType(m)) {
        msgAlert("市场分类信息" + WCMLang.Delete_success);
        reloadMarketTypeList();
    }
    else {
        msgWargin("市场分类信息" + WCMLang.Delete_fail);
    }
}
...
function updateMarketTypeData() {
    ...
    if (bool) {
        msgAlert("市场分类信息保存成功");
        getCurrentFrameObj(topnum).reloadMarketTypeList();
        CloseModalWindow();
    } else {
        msgWargin("市场分类信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("市场分类信息", "/sys/project/priceMonitor/viewMarketType.jsp?topnum="+curTabIndex, 485, 303);
}
...
function changeStatus(value) {
    ...
    if (MarketTypeRPC.changeStatus(m)) {
        msgAlert("市场分类信息状态设置成功");
        reloadMarketTypeList();
    } else {
        msgWargin("市场分类信息状态设置失败，请重新操作");
    }
}

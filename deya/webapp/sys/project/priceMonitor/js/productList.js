...
function openUpdateProductPage(id) {
    ...
    OpenModalWindow("蔬果产品信息", "/sys/project/priceMonitor/viewProduct.jsp?id=" + c_id + "&topnum="+curTabIndex, 485, 363);
}
...
function deleteProduct() {
    ...
    if (ProductRPC.deleteProduct(m)) {
        msgAlert("蔬果产品信息" + WCMLang.Delete_success);
        reloadProductList();
    }
    else {
        msgWargin("蔬果产品信息" + WCMLang.Delete_fail);
    }
}
...
function updateProductData() {
    ...
    if (bool) {
        msgAlert("蔬果产品信息保存成功");
        getCurrentFrameObj(topnum).reloadProductList();
        CloseModalWindow();
    } else {
        msgWargin("蔬果产品信息保存失败，请重新操作");
    }
}

function addInfo() {
    OpenModalWindow("蔬果产品信息", "/sys/project/priceMonitor/viewProduct.jsp?topnum="+curTabIndex, 485, 363);
}
...
function changeStatus(value) {
    ...
    if (ProductRPC.changeStatus(m)) {
        msgAlert("蔬果产品信息状态设置成功");
        reloadProductList();
    } else {
        msgWargin("蔬果产品信息状态设置失败，请重新操作");
    }
}

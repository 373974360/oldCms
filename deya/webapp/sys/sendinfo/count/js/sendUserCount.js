...
function initTable(){
	...
	colsList.add(setTitleClos("user_realname","报送人","","","",""));
...
function showList(){
	...
	beanList = SendInfoRPC.getSendRecordUserCount(m);//第一个参数为站点ID，暂时默认为空	
...
function getReceiveSiteListForRecord()
{
	...
	if(l != null && l.size() > 0)
	{
		for (var i = 0; i < l.size(); i++) {
			tableTr += '<li><input style="vertical-align:middle" type="checkbox" id="b_id" name="b_id" onclick="setAllState()" value="'+l.get(i).t_site_id+'"><label  >'+l.get(i).t_site_name+'</label></li>';
		}
		$("#b_tr").append(tableTr); 
	}
}

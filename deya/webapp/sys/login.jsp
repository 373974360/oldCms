...
<%
	...
	if(user_name != null && !"".equals(user_name))
	{
		...
		{
			out.println("<script>alert(\"验证码不正确\");changeCreateImage();</script>");
			return;
		}
		else
		{
			if(!"0".equals(res))		
			{
				...
				switch(msg)
				{
					case 1:out.println("<script>alert(\"该帐号还未开通\");changeCreateImage();</script>");break;
					case 2:out.println("<script>alert(\"该用户已被停用\");changeCreateImage();</script>");break;
					case 3:out.println("<script>alert(\"用户名密码不正确\");changeCreateImage();</script>");break;//用户名不正确
					case 4:out.println("<script>alert(\"用户名密码不正确\");changeCreateImage();</script>");break;//用户名密码不正确
					case 5:out.println("<script>alert(\"该用户不存在\");changeCreateImage();</script>");break;//
					default:out.println("<script>alert(\"用户名密码不正确\");changeCreateImage();</script>");break;
				}
...
.mbox .login{ height:45px; line-height:45px;background:#0056ac;  width:100%; margin:0 auto; text-align:center; padding-top:15px; }

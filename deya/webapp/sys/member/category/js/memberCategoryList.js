...
function showList(){
	...
	table.getCol("mcat_name").each(function(i){	
		if(i>0)
		{
			...
			$(this).html('<a href="javascript:OpenModalWindow(\'会员分类信息\',\'/sys/member/category/memberCategory_add.jsp?type=update&mcat_id='+id +'\',450,245)">'+beanList.get(i-1).mcat_name+'</a>');
		}
	});
...
{
	OpenModalWindow("新建会员分类信息","/sys/member/category/memberCategory_add.jsp?type=add&mcat_id=",460,245);
}
...
function updateMemberCategory()
{
	...
	OpenModalWindow("修改会员分类信息","/sys/member/category/memberCategory_add.jsp?type=update&mcat_id="+id,460,245);
}
...
function deleteMemberCategory()
{
	...
	{
		msgAlert("会员分类信息"+WCMLang.Delete_success);
		reloadCategoryList();
	}
	else
	{
		msgWargin("会员分类信息"+WCMLang.Delete_fail);
	}
}
...
function selectSiteUser(mcat_id)
{
    ...
    //openSelectSiteUserPage("选择用户","insertMemberReleUserByCat",site_id);
    OpenModalWindow("选择用户","/sys/org/siteUser/select_site_user.jsp?app_id=cms&site_id="+current_site_id+"&handl_name=insertMemberReleUserByCat",520,515);
}
...
function getSelectedUserIDS()
{
    current_user_ids = "";
    current_group_ids = "";
    var l = MemberManRPC.getMemberReleUserListByCat(temp_mcat_id,current_site_id);
    l = List.toJSList(l);
    if(l != null && l.size() > 0)
    {
...
{
    if(MemberManRPC.insertMemberReleUserByCat(temp_mcat_id,user_ids,group_ids,"cms",current_site_id))
    {
        msgAlert("会员分类用户关联"+WCMLang.Add_success);
    }else
    {
        msgWargin("会员分类用户关联"+WCMLang.Add_fail);
    }
}
/********** 关联用户 结束 **********/
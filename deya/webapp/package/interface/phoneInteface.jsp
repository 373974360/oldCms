...
<%@page import="com.deya.wcm.services.survey.AnswerServer" %><%@ page import="java.util.regex.Pattern"%><%@ page import="java.util.regex.Matcher"%>
<%
...
public String getNewsList(HttpServletRequest request)
{
	...
	if(info_list != null && info_list.size() > 0)
	{
		...
		{
			json += ",{\"info_id\":\""+info.getInfo_id()+"\",\"title\":\""+replaceFont(info.getTitle())+"\",\"model_id\":\""+info.getModel_id()+"\",\"content_url\":\""+info.getContent_url()+"\",\"description\":\""+replaceStr(info.getDescription())+"\",\"source\":\""+info.getSource()+"\",\"released_dtime\":\""+info.getReleased_dtime()+"\", \"thumb_url\":\""+info.getThumb_url()+"\"}";
		}
...
public String SurveyView(HttpServletRequest request){
	...
	if(surveyBean!=null){
	    json += "{\"s_id\":\""+surveyBean.getS_id()+"\",\"s_name\":\""+delHTMLTag(surveyBean.getS_name())+"\",\"is_end\":\""+surveyBean.getIs_end()+"\",\"show_result_status\":\""+surveyBean.getShow_result_status()+"\",\"start_time\":\""+surveyBean.getStart_time()+"\",\"end_time\":\""+surveyBean.getEnd_time()+"\",\"s_description\":\""+delHTMLTag(surveyBean.getDescription())+"\",\"subjectList\":[";
	    List<SurveySub> subList = SurveyService.getSurveySubjectBean(surveyBean.getS_id());
	    if(subList != null && subList.size() > 0){
	        for(SurveySub sub:subList){
	            json+="{\"subject_id\":\""+sub.getSubject_id()+"\",\"sub_name\":\""+delHTMLTag(sub.getSub_name())+"\",\"subject_type\":\""+sub.getSubject_type()+"\",\"is_required\":\""+sub.getIs_required()+"\",\"childList\":[";

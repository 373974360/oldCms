...
public class InfoDAO {
     ...
     * @return boolean
     */
    public static boolean backPassInfoStatus(String info_id, String info_status, String step_id, SettingLogsBean stl) {
        Map<String, String> m = new HashMap<String, String>();
        m.put("info_id", info_id);
        m.put("step_id", step_id);
        m.put("opt_dtime", DateUtil.getCurrentDateTime());
        m.put("info_status", info_status);
        if (DBManager.update("backPassInfoStatus", m)) {
            PublicTableDAO.insertSettingLogs("撤回", "信息状态撤回", info_id, stl);
            return true;
        } else
            return false;
    }
    ...
    public static boolean clearTrach(String cat_ids, String app_id, String site_id, SettingLogsBean stl) {
        try {
            ...
            if (model_list != null && model_list.size() > 0) {
                for (ModelBean mb : model_list) {//链接类型的没副表

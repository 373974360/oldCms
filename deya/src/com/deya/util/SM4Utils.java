...
public class SM4Utils {
    ...
    public static boolean verifyEcb(String cipherText,String paramStr) throws Exception{
        ...
        byte[] keyData = ByteUtils.fromHexString(KEY);
        //将16进制字符串转换成数组
        byte[] ciperhData = ByteUtils.fromHexString(cipherText);
        //解密
        byte[] decryptData = decrypt_Ecb_Padding(keyData,ciperhData);
        //将原字符串转换成byte[]
        byte[] srcData = paramStr.getBytes(ENCODING);
        //判断2个数组是否一致
    ...
    public static void main(String[] args){
        try {
            String pwd="123456";
            String cipher = SM4Utils.encryptEcb(pwd);//加密
            System.out.println(cipher);
            System.out.println(SM4Utils.verifyEcb(cipher,pwd));

            String pwd_s = SM4Utils.decryptEcb(cipher);

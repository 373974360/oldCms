...
function openUpdatePriceInfoPage(id) {
    ...
    top.OpenModalWindow("价格监测信息", "/sys/project/priceMonitor/viewPriceInfo.jsp?id=" + c_id + "&topnum="+top.curTabIndex, 485, 450);
}
...
function deletePriceInfo() {
    ...
    if (PriceInfoRPC.deletePriceInfo(m)) {
        top.msgAlert("价格监测信息" + WCMLang.Delete_success);
        reloadPriceInfoList();
    }
    else {
        top.msgWargin("价格监测信息" + WCMLang.Delete_fail);
    }
}
...
function updatePriceInfoData() {
    ...
    if (id != null && id != "") {
        bean.id = id;
        bean.addUser = top.LoginUserBean.user_id;
        if(bean.marketId == 2)
        {
            bean.marketId = 1;
        }
    ...
    if (bool) {
        top.msgAlert("价格监测信息保存成功");
        top.getCurrentFrameObj(topnum).reloadPriceInfoList();
        top.CloseModalWindow();
        if(isAdd)
        {
            top.getCurrentFrameObj(topnum).addInfo();
        }
    } else {
        top.msgWargin("价格监测信息保存失败，请重新操作");
    }
}

function addInfo() {
    top.OpenModalWindow("价格监测信息", "/sys/project/priceMonitor/viewPriceInfo.jsp?topnum="+top.curTabIndex, 485, 450);
}

...
function openUpdateProductTypePage(id) {
    ...
    top.OpenModalWindow("品种分类信息", "/sys/project/priceMonitor/viewProductType.jsp?id=" + c_id + "&topnum="+top.curTabIndex, 485, 338);
}
...
function deleteProductType() {
    ...
    if (ProductTypeRPC.deleteProductType(m)) {
        top.msgAlert("品种分类信息" + WCMLang.Delete_success);
        reloadProductTypeList();
    }
    else {
        top.msgWargin("品种分类信息" + WCMLang.Delete_fail);
    }
}
...
function updateProductTypeData() {
    ...
    if (bool) {
        top.msgAlert("品种分类信息保存成功");
        top.getCurrentFrameObj(topnum).reloadProductTypeList();
        top.CloseModalWindow();
    } else {
        top.msgWargin("品种分类信息保存失败，请重新操作");
    }
}

function addInfo() {
    top.OpenModalWindow("品种分类信息", "/sys/project/priceMonitor/viewProductType.jsp?topnum="+top.curTabIndex, 485, 338);
}
...
function changeStatus(value) {
    ...
    if (ProductTypeRPC.changeStatus(m)) {
        top.msgAlert("品种分类信息状态设置成功");
        reloadProductTypeList();
    } else {
        top.msgWargin("品种分类信息状态设置失败，请重新操作");
    }
}

...
function openUpdateYjcpzsPage(id) {
    ...
    top.OpenModalWindow("有机产品认证证书信息", "/sys/project/searchInfo/viewYjcpzs.jsp?id=" + c_id + "&topnum="+top.curTabIndex, 605, 533);
}
...
function deleteYjcpzs() {
    ...
    if (YjcpzsRPC.deleteYjcpzs(m)) {
        top.msgAlert("有机产品认证证书信息" + WCMLang.Delete_success);
        reloadYjcpzsList();
    }
    else {
        top.msgWargin("有机产品认证证书信息" + WCMLang.Delete_fail);
    }
}
...
function updateYjcpzsData() {
    ...
    if (bool) {
        top.msgAlert("有机产品认证证书信息保存成功");
        top.getCurrentFrameObj(topnum).reloadYjcpzsList();
        top.CloseModalWindow();
    } else {
        top.msgWargin("有机产品认证证书信息保存失败，请重新操作");
    }
}

function addInfo() {
    top.OpenModalWindow("有机产品认证证书信息", "/sys/project/searchInfo/viewYjcpzs.jsp?topnum="+top.curTabIndex, 605, 533);
}
...
function changeStatus(value) {
    ...
    if (YjcpzsRPC.changeStatus(m)) {
        top.msgAlert("有机产品认证证书信息状态设置成功");
        reloadYjcpzsList();
    } else {
        top.msgWargin("有机产品认证证书信息状态设置失败，请重新操作");
    }
}

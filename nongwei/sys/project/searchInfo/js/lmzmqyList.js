...
function openUpdateLmzmqyPage(id) {
    ...
    top.OpenModalWindow("林木种苗企业信息", "/sys/project/searchInfo/viewLmzmqy.jsp?id=" + c_id + "&topnum="+top.curTabIndex, 485, 483);
}
...
function deleteLmzmqy() {
    ...
    if (LmzmqyRPC.deleteLmzmqy(m)) {
        top.msgAlert("林木种苗企业信息" + WCMLang.Delete_success);
        reloadLmzmqyList();
    }
    else {
        top.msgWargin("林木种苗企业信息" + WCMLang.Delete_fail);
    }
}
...
function updateLmzmqyData() {
    ...
    if (bool) {
        top.msgAlert("林木种苗企业信息保存成功");
        top.getCurrentFrameObj(topnum).reloadLmzmqyList();
        top.CloseModalWindow();
    } else {
        top.msgWargin("林木种苗企业信息保存失败，请重新操作");
    }
}

function addInfo() {
    top.OpenModalWindow("林木种苗企业信息", "/sys/project/searchInfo/viewLmzmqy.jsp?topnum="+top.curTabIndex, 485, 483);
}
...
function changeStatus(value) {
    ...
    if (LmzmqyRPC.changeStatus(m)) {
        top.msgAlert("林木种苗企业信息状态设置成功");
        reloadLmzmqyList();
    } else {
        top.msgWargin("林木种苗企业信息状态设置失败，请重新操作");
    }
}

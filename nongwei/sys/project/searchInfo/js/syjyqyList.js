...
function openUpdateSyjyqyPage(id) {
    ...
    top.OpenModalWindow("通过兽药GSP兽药经营企业信息", "/sys/project/searchInfo/viewSyjyqy.jsp?id=" + c_id + "&topnum="+top.curTabIndex, 485, 433);
}
...
function deleteSyjyqy() {
    ...
    if (SyjyqyRPC.deleteSyjyqy(m)) {
        top.msgAlert("通过兽药GSP兽药经营企业信息" + WCMLang.Delete_success);
        reloadSyjyqyList();
    }
    else {
        top.msgWargin("通过兽药GSP兽药经营企业信息" + WCMLang.Delete_fail);
    }
}
...
function updateSyjyqyData() {
    ...
    if (bool) {
        top.msgAlert("通过兽药GSP兽药经营企业信息保存成功");
        top.getCurrentFrameObj(topnum).reloadSyjyqyList();
        top.CloseModalWindow();
    } else {
        top.msgWargin("通过兽药GSP兽药经营企业信息保存失败，请重新操作");
    }
}

function addInfo() {
    top.OpenModalWindow("通过兽药GSP兽药经营企业信息", "/sys/project/searchInfo/viewSyjyqy.jsp?topnum="+top.curTabIndex, 485, 433);
}
...
function changeStatus(value) {
    ...
    if (SyjyqyRPC.changeStatus(m)) {
        top.msgAlert("通过兽药GSP兽药经营企业信息状态设置成功");
        reloadSyjyqyList();
    } else {
        top.msgWargin("通过兽药GSP兽药经营企业信息状态设置失败，请重新操作");
    }
}
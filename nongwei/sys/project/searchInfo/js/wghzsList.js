...
function openUpdateWghzsPage(id) {
    ...
    top.OpenModalWindow("无公害证书信息", "/sys/project/searchInfo/viewWghzs.jsp?id=" + c_id + "&topnum="+top.curTabIndex, 485, 473);
}
...
function deleteWghzs() {
    ...
    if (WghzsRPC.deleteWghzs(m)) {
        top.msgAlert("无公害证书信息" + WCMLang.Delete_success);
        reloadWghzsList();
    }
    else {
        top.msgWargin("无公害证书信息" + WCMLang.Delete_fail);
    }
}
...
function updateWghzsData() {
    ...
    if (bool) {
        top.msgAlert("无公害证书信息保存成功");
        top.getCurrentFrameObj(topnum).reloadWghzsList();
        top.CloseModalWindow();
    } else {
        top.msgWargin("无公害证书信息保存失败，请重新操作");
    }
}

function addInfo() {
    top.OpenModalWindow("无公害证书信息", "/sys/project/searchInfo/viewWghzs.jsp?topnum="+top.curTabIndex, 485, 473);
}
...
function changeStatus(value) {
    ...
    if (WghzsRPC.changeStatus(m)) {
        top.msgAlert("无公害证书信息状态设置成功");
        reloadWghzsList();
    } else {
        top.msgWargin("无公害证书信息状态设置失败，请重新操作");
    }
}

...
function openUpdateCwmzyyPage(id) {
    ...
    top.OpenModalWindow("宠物医院及门诊名单", "/sys/project/searchInfo/viewCwmzyy.jsp?id=" + c_id + "&topnum="+top.curTabIndex, 485, 433);
}
...
function deleteCwmzyy() {
    ...
    if (CwmzyyRPC.deleteCwmzyy(m)) {
        top.msgAlert("宠物医院及门诊名单" + WCMLang.Delete_success);
        reloadCwmzyyList();
    }
    else {
        top.msgWargin("宠物医院及门诊名单" + WCMLang.Delete_fail);
    }
}
...
function updateCwmzyyData() {
    ...
    if (bool) {
        top.msgAlert("宠物医院及门诊名单保存成功");
        top.getCurrentFrameObj(topnum).reloadCwmzyyList();
        top.CloseModalWindow();
    } else {
        top.msgWargin("宠物医院及门诊名单保存失败，请重新操作");
    }
}

function addInfo() {
    top.OpenModalWindow("宠物医院及门诊名单", "/sys/project/searchInfo/viewCwmzyy.jsp?topnum="+top.curTabIndex, 485, 433);
}
...
function changeStatus(value) {
    ...
    if (CwmzyyRPC.changeStatus(m)) {
        top.msgAlert("宠物医院及门诊名单状态设置成功");
        reloadCwmzyyList();
    } else {
        top.msgWargin("宠物医院及门诊名单状态设置失败，请重新操作");
    }
}

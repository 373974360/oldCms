...
function setHtmlEditButton(textareaID)
{
	...
	else
	{
		...
		var str = '<div style="padding-top:3px"><input id="tool_btn"  type="button" value="一键排版" onclick="formatStyle(\'yjpb\',\''+textareaID+'\')"  style="cursor:hand"/></div>';
...
function clearStyleByTag(str)
{
	try{
		...
		str = str.replace( /\s*border-collapse:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*width:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*border:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*font-family:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*font-size:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*color:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*text-indent:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*text-align:left([;|'|"])/gi, '' ) ;
		str = str.replace( /\s*margin-left:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*margin-right:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*vertical-align:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*margin:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*vertical-align:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*vertical-align:([^;]+[;|'|"])/gi, '' ) ;
		str = str.replace( /\s*vertical-align:([^;]+[;|'|"])/gi, '' ) ;
		
		str = str.replace( /\s*<table/gi,'<table align="center" border="1" bordercolor="#000000" ');
		
		str = str.replace(/(style=\s*[\"|\']\s*[\"|\'])/ig,"") ;
		
		str = str.replace( /(\s*<img)(.*)(\/>)/gi, '<p align="center">$1$2$3</p>' ) ;
		
		str = str.replace( /\s*&nbsp;/gi, '') ;
		//str = str.replace(/(<p>)((\n|\r|.)*?)/ig, '<p style="text-indent:2em;">') ;
		/*
		str = str.replace(/<(\/)?h[1-9]>/g,"<$1p>").replace(/(<[^>]*?)(center=\s*[\"|\'].*?\s*[\"|\'])([^>]*>)/ig,"$1$3").replace(/<(\/)?h[1-9]>/g,"<$1p>");
		*/
		return str;
	}catch(e){
		return str;

...
public class CmsCountDAO {
     ...
     */
    @SuppressWarnings("unchecked")
    public static Map<String, CmsCountBean> getMemberInputCountList(Map<String, String> mp){

        List<Map> result = DBManager.queryFList("getMemberInputCountList", mp);

        Map<String, CmsCountBean> ret = new HashMap<String, CmsCountBean>();
        for(Map m : result ){
            String count = m.get("COUNT").toString(); // 取得返回的count列
            String input_user = m.get("INPUT_USER").toString();
            String user_name = m.get("ME_REALNAME").toString();
            CmsCountBean cmb = new CmsCountBean(input_user, user_name, count);
            ret.put(input_user, cmb);
        }
        return ret;

package com.deya.project.searchInfo;

import com.deya.util.DateUtil;
import com.deya.util.FormatUtil;
import com.deya.wcm.bean.logs.SettingLogsBean;
import com.deya.wcm.catchs.ISyncCatch;
import com.deya.wcm.dao.PublicTableDAO;

import java.util.*;

public class SyjyqyManager implements ISyncCatch {

    public static TreeMap<Integer,SyjyqyBean> ptMap = new TreeMap<Integer,SyjyqyBean>();

    static{
        reloadCatchHandl();
    }

    public void reloadCatch()
    {
        reloadCatchHandl();
    }

    public static void reloadCatchHandl()
    {
        ptMap.clear();
        try{
            Map<String,String> m = new HashMap<String,String>();
            List<SyjyqyBean> pdbList = SyjyqyDAO.getAllSyjyqy();
            if(pdbList != null && pdbList.size() > 0)
            {
                for(int i=0;i<pdbList.size();i++)
                {
                    ptMap.put(pdbList.get(i).getId(), pdbList.get(i));
                }
            }
        }catch(Exception e)
        {
            e.printStackTrace();
        }
    }

    public static String getSyjyqyCount(Map<String, String> m) {
        if (m.containsKey("key_word")) {
            if (!FormatUtil.isValiditySQL((String) m.get("key_word")))
                return "0";
        }
        return SyjyqyDAO.getSyjyqyCount(m);
    }

    public static List<SyjyqyBean> getSyjyqyList(Map<String, String> m) {
        if (m.containsKey("key_word")) {
            if (!FormatUtil.isValiditySQL((String) m.get("key_word")))
                return new ArrayList();
        }
        return SyjyqyDAO.getSyjyqyList(m);
    }

    public static SyjyqyBean getSyjyqyBean(String id) {

        return ptMap.get(Integer.parseInt(id));
    }

    public static boolean changeStatus(Map<String, String> m, SettingLogsBean stl) {
        if(SyjyqyDAO.changeStatus(m, stl))
        {
            reloadCatchHandl();
            return true;
        }
        return false;
    }

    public static boolean insertSyjyqy(SyjyqyBean hb, SettingLogsBean stl) {
        hb.setId(PublicTableDAO.getIDByTableName("dz_syjyqy"));
        hb.setAddTime(DateUtil.getCurrentDateTime());
        if(SyjyqyDAO.insertSyjyqy(hb, stl))
        {
            reloadCatchHandl();
            return true;
        }
        return false;
    }


    public static boolean updateSyjyqy(SyjyqyBean hb, SettingLogsBean stl) {
        if(SyjyqyDAO.updateSyjyqy(hb, stl))
        {
            reloadCatchHandl();
            return true;
        }
        return false;
    }

    public static boolean deleteSyjyqy(Map<String, String> m, SettingLogsBean stl) {
        if(SyjyqyDAO.deleteSyjyqy(m, stl))
        {
            reloadCatchHandl();
            return true;
        }
        return false;
    }
}